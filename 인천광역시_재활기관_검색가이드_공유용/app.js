(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("incheon-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // 10 districts/counties of Incheon Metropolitan City
  const districtOrder = [
    "중구", "동구", "미추홀구", "연수구", "남동구", 
    "부평구", "계양구", "서구", "강화군", "옹진군"
  ];
  ...
  // Filter out official portals and return hospital specific homepage
  ...
  // Custom Naver search URL for hospital search
  const homepageSearchUrl = (item) =>
    `https://search.naver.com/search.naver?query=${encodeURIComponent(item.name + " " + item.district)}`;
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  // Generates source links, ensuring there is ALWAYS at least one Naver Search fallback link at the end
  function sourceLinks(item) {
    const urls = sourceUrls(item);
    const linksMarkup = urls.map((url, index) => {
      let label = `출처 ${index + 1} ↗`;
      if (/hira\.or\.kr/i.test(url)) label = "심평원 정보원 ↗";
      else if (/comwel\.or\.kr/i.test(url)) label = "근로복지공단 ↗";
      else if (/nrc\.go\.kr/i.test(url)) label = "국립재활원 ↗";
      else if (/mohw\.go\.kr/i.test(url)) label = "보건복지부 ↗";
      else label = "공식 홈페이지 ↗";
      return `<a href="${escapeHtml(url)}" target="_blank" rel="noopener">${label}</a>`;
    });
    
    // Always append Naver Search Link as the final resource
    const searchUrl = homepageSearchUrl(item);
    linksMarkup.push(`<a href="${escapeHtml(searchUrl)}" target="_blank" rel="noopener" style="color: var(--mint); font-weight: 800;">네이버 병원 검색 ↗</a>`);
    ...
    elements.dialogContent.innerHTML = `
      ...
      <div class="dialog-body">
        ...
        <dl class="detail-list">
          ${detailRows(item)}
          <dt>근거 자료</dt><dd class="source-links">${sourceLinks(item)}</dd>
        </dl>
      </div>`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("incheon-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // Ignore local storage error in local files
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `인천 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
