(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("chungnam-daejeon-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // 20 districts/counties of Chungnam-Daejeon
  const districtOrder = [
    "서구", "유성구", "중구", "동구", "대덕구", 
    "천안시", "아산시", "서산시", "당진시", "공주시", 
    "논산시", "보령시", "계룡시", "홍성군", "예산군", 
...
* 관련 유튜브: ${youtubeUrl(item)}`;
    ...
    return `
      <article class="institution-card" data-id="${escapeHtml(item.id)}">
        ...
        <div class="card-actions">
          ...
          <a href="${escapeHtml(homepage || homepageSearchUrl(item))}" target="_blank" rel="noopener" data-action="homepage-select" data-id="${escapeHtml(item.id)}">${homepage ? "홈페이지" : "홈페이지 찾기"}</a>
    ...
    elements.dialogContent.innerHTML = `
      ...
      <div class="dialog-body">
        <div class="dialog-actions">
          ...
          <a href="${escapeHtml(homepage || homepageSearchUrl(item))}" target="_blank" rel="noopener" data-action="homepage-select" data-id="${escapeHtml(item.id)}">${homepage ? "공식 홈페이지 ↗" : "홈페이지 찾기 ↗"}</a>
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("chungnam-daejeon-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function openHomepageSelect(item) {
    ...
    if (elements.homepageDialogContent) {
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `경북 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
  ...
  elements.dialog?.addEventListener("click", async (event) => {
    if (elements.dialog && (event.target === elements.dialog || event.target.closest(".dialog-close"))) elements.dialog.close();
  ...
  elements.homepageDialog?.addEventListener("click", (event) => {
    if (elements.homepageDialog && (event.target === elements.homepageDialog || event.target.closest(".homepage-dialog-close"))) {
