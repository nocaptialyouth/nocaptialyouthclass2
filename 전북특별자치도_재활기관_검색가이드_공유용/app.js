(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("jeonbuk-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // Custom ordering for Jeonbuk districts
  const districtOrder = [
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function presetMatches(item, preset) {
    ...
    const rules = {
      ...
      dayRehab: () => /운영|확인/.test(item.dayRehab) && !/미운영|해당없음/.test(item.dayRehab),
    };
  ...
  function tagsFor(item) {
    ...
    if (/운영|확인/.test(item.dayRehab) && !/미운영|해당없음/.test(item.dayRehab)) tags.push("낮병동");
  ...
  function cardTemplate(item) {
    ...
    const phoneLink = isPhone(item.phone)
      ? `<a href="tel:${escapeHtml(item.phone)}" class="tel-btn" data-phone="${escapeHtml(item.phone)}" data-name="${escapeHtml(item.name)}">전화하기</a>`
      : ``;
  ...
  function detailRows(item) {
    ...
    return fields.map(([term, description]) =>
      `<dt class="detail-label">${escapeHtml(term)}</dt><dd class="detail-value">${escapeHtml(description || "정보 없음")}</dd>`
    ).join("");
  }
  ...
  function sourceLinks(sources) {
    ...
    return urls.length
      ? urls.map((url, index) => `<a href="${escapeHtml(url)}" target="_blank" rel="noopener" style="margin-right: 8px; color: var(--teal-600); font-weight: 700; text-decoration: none;">출처 ${index + 1} ↗</a>`).join("")
      : "등록된 링크 없음";
  }

  function detailNotice(item) {
    if (/지정/.test(item.recovery)) {
      return '<span class="dialog-notice" style="background: var(--teal-600); color: var(--white); font-weight: 800; font-size: 11px; padding: 4px 10px; margin-bottom: 8px; display: inline-block; border-radius: 6px; letter-spacing: 0;">💡 보건복지부 지정 제3기 회복기 재활의료기관</span>';
    }
    if (/한방병원|한의원/.test(item.type)) {
      return '<span class="dialog-notice" style="background: #f1f5f9; color: #475569; font-weight: 800; font-size: 11px; padding: 4px 10px; margin-bottom: 8px; display: inline-block; border-radius: 6px;">한방재활 후보 · 의과 재활의학과와 구분</span>';
    }
    if (item.rehabDept === "있음") {
      return '<span class="dialog-notice" style="background: var(--mint-soft); color: var(--teal-600); font-weight: 800; font-size: 11px; padding: 4px 10px; margin-bottom: 8px; display: inline-block; border-radius: 6px;">✓ 재활의학과 확인 기관</span>';
    }
    return '<span class="dialog-notice" style="background: var(--amber-soft); color: var(--amber); font-weight: 800; font-size: 11px; padding: 4px 10px; margin-bottom: 8px; display: inline-block; border-radius: 6px;">☎ 최신 진료·입원 운영 여부 전화 확인 권장</span>';
  }

  function openDetail(item) {
    activeDetailItem = item;
    const phoneAction = isPhone(item.phone)
      ? `<a href="tel:${escapeHtml(item.phone)}" data-phone="${escapeHtml(item.phone)}" data-name="${escapeHtml(item.name)}" style="background: var(--teal-900); color: white; border-color: var(--teal-900);">☎ 전화번호 보기</a>`
      : ``;
    const homepage = homepageUrl(item);
    elements.dialogContent.innerHTML = `
      <div class="modal-header">
        <div class="dialog-title-wrap">
          ${detailNotice(item)}
          <p style="margin: 4px 0 0; font-size: 12px; opacity: 0.8; font-weight: 700; letter-spacing: 0.05em;">${escapeHtml(item.district)} · ${escapeHtml(item.type)}</p>
          <h2 id="dialogTitle" style="color: white; margin: 6px 0 0; font-size: 24px;">${escapeHtml(item.name)}</h2>
        </div>
        <button class="close-modal dialog-close" aria-label="닫기">×</button>
      </div>
      <div class="modal-body">
        <div class="dialog-actions" style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 24px;">
          ${phoneAction}
          <a href="${mapUrl(item)}" target="_blank" rel="noopener" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 700; color: var(--ink);">네이버 지도 ↗</a>
          <a href="${kakaoMapUrl(item)}" target="_blank" rel="noopener" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 700; color: var(--ink);">카카오맵 ↗</a>
          ${homepage
            ? `<a href="#" data-action="homepage-select" data-id="${escapeHtml(item.id)}" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 700; color: var(--ink);">공식 홈페이지 ↗</a>`
            : `<a href="${escapeHtml(homepageSearchUrl(item))}" target="_blank" rel="noopener" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 700; color: var(--ink);">홈페이지 찾기 ↗</a>`}
          <a href="${youtubeUrl(item)}" target="_blank" rel="noopener" style="padding: 8px 12px; border: 1px solid #ffd6d6; background: #fff5f5; border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 700; color: #e50914;">유튜브 검색 ↗</a>
          <button type="button" data-copy="${escapeHtml(item.address)}" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; font-size: 12px; font-weight: 700; background: white;">주소 복사</button>
          <button type="button" data-share="${escapeHtml(item.id)}" style="padding: 8px 12px; border: 1px solid var(--line); border-radius: 8px; font-size: 12px; font-weight: 700; background: white;">정보 공유</button>
        </div>
        <dl class="detail-grid" style="display: grid; grid-template-columns: 140px 1fr; row-gap: 12px; margin: 0; padding: 0;">
          ${detailRows(item)}
          <dt class="detail-label">근거 자료</dt><dd class="detail-value source-links">${sourceLinks(item.sources)}</dd>
        </dl>
      </div>`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("jeonbuk-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // Local storage block bypass for security
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.className = "district-chip" + (state.district === "" ? " active" : "");
    allChip.dataset.district = "";
    allChip.innerHTML = `전북 전체 <span style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
    row.appendChild(allChip);
    
    districtOrder.forEach((gugun) => {
      if (!counts[gugun]) return;
      const chip = document.createElement("button");
      chip.className = "district-chip" + (state.district === gugun ? " active" : "");
      chip.dataset.district = gugun;
      chip.innerHTML = `${gugun} <span style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${counts[gugun]}</span>`;
      row.appendChild(chip);
    });
    
    row.querySelectorAll(".district-chip").forEach((el) => {
      el.addEventListener("click", () => {
        row.querySelectorAll(".district-chip").forEach((x) => x.classList.remove("active"));
  ...
  function resetAll() {
    ...
    document.querySelectorAll("#districtRow .district-chip").forEach((el) => {
