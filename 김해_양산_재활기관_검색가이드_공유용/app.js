(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("gimhae-yangsan-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("gimhae-yangsan-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
