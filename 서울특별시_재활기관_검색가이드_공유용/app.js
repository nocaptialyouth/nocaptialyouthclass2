(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("seoul-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // Extract user-friendly custom homepage, ignoring portal site databases (HIRA, Comwel, KARM, etc.)
  const homepageUrl = (item) => sourceUrls(item).find((url) =>
    !/hira\.or\.kr|comwel\.or\.kr|karm\.or\.kr|mohw\.go\.kr|kmspecialist\.org|ddoga\.co\.kr|caredoc\.kr/i.test(url)
  );

  // Redirection link when custom homepage is absent
  ...
  const getShareText = (item) => {
    ...
    return `[서울 재활기관 정보 공유]
■ 기관명: ${item.name} (${item.type})
■ 자치구: 서울특별시 ${item.district}
...
* 홈페이지: ${homepage || homepageSearchUrl(item)}
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function tagsFor(item) {
    ...
    if (/지정/.test(item.recovery)) tags.push("회복기 지정");
    if (/인증|공단/.test(item.workersComp)) tags.push("산재 인증");
    if (/가능/.test(item.autoInsurance) && !/해당없음|확인/.test(item.autoInsurance)) tags.push("자보 가능");
    if (/소아|어린이/.test(item.conditions + " " + item.specialty)) tags.push("소아재활");
    if (/투석|인공신장/.test(item.notes + " " + item.conditions)) tags.push("투석가능");
    if (/운영 확인|가능성 높음|운영/.test(item.dayRehab)) tags.push("낮병동");
  ...
  function cardTemplate(item) {
    ...
    const phoneLink = isPhone(item.phone)
      ? `<a href="tel:${escapeHtml(item.phone)}" data-phone="${escapeHtml(item.phone)}" data-name="${escapeHtml(item.name)}">전화하기</a>`
      : ``;
    const homepage = homepageUrl(item);
    ...
    return `
      <article class="institution-card" data-id="${escapeHtml(item.id)}">
        ...
        <div class="card-actions">
          ${phoneLink}
          <a href="${mapUrl(item)}" target="_blank" rel="noopener">지도보기</a>
          <a href="#" data-action="homepage-select" data-id="${escapeHtml(item.id)}" class="btn-link-action">${homepage ? "홈페이지" : "홈페이지 검색"}</a>
          <button class="detail-button" data-action="detail">상세보기</button>
        </div>
      </article>`;
  }
  ...
  function openDetail(item) {
    ...
    const phoneAction = isPhone(item.phone)
      ? `<a href="tel:${escapeHtml(item.phone)}" data-phone="${escapeHtml(item.phone)}" data-name="${escapeHtml(item.name)}">☎ 전화 걸기</a>`
      : ``;
    const homepage = homepageUrl(item);
    elements.dialogContent.innerHTML = `
      <div class="dialog-header">
        <div class="dialog-title-wrap">
          ${detailNotice(item)}
          <p>서울특별시 ${escapeHtml(item.district)} · ${escapeHtml(item.type)}</p>
      ...
      <div class="dialog-body">
        <div class="dialog-actions">
          ...
          <a href="#" data-action="homepage-select" data-id="${escapeHtml(item.id)}" class="btn-link-action">${homepage ? "공식 홈페이지 ↗" : "홈페이지 검색 ↗"}</a>
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("seoul-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `서울 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
