(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("jeollagwangju-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // Custom ordering for Gwangju, Jeonnam, and Jeonbuk districts
  const districtOrder = [
    "광산구", "북구", "남구", "서구", "동구",
    "순천시", "목포시", "여수시", "광양시",
    "전주시", "익산시", "군산시"
...
* 관련 유튜브: ${youtubeUrl(item)}`;
    ...
    return `
      <article class="institution-card" data-id="${escapeHtml(item.id)}">
        ...
        <div class="card-actions">
          ...
          ${homepage
            ? `<a href="#" data-action="homepage-select" data-id="${escapeHtml(item.id)}" class="btn-link-action">홈페이지</a>`
            : `<a href="${escapeHtml(homepageSearchUrl(item))}" target="_blank" rel="noopener" class="btn-link-action">홈페이지 찾기</a>`}
    ...
    elements.dialogContent.innerHTML = `
      ...
      <div class="dialog-body">
        <div class="dialog-actions">
          ...
          ${homepage
            ? `<a href="#" data-action="homepage-select" data-id="${item.id}" class="btn-link-action">공식 홈페이지 ↗</a>`
            : `<a href="${escapeHtml(homepageSearchUrl(item))}" target="_blank" rel="noopener" class="btn-link-action">홈페이지 찾기 ↗</a>`}
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("jeollagwangju-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `전라광주 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
