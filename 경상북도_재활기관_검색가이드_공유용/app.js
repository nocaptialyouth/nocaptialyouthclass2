(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("gyeongbuk-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // 22 districts/counties of Gyeongsangbuk-do
  const districtOrder = [
    "포항시", "경주시", "김천시", "안동시", "구미시", 
    "영주시", "영천시", "상주시", "문경시", "경산시", 
    "의성군", "청송군", "영양군", "영덕군", "청도군", 
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("gyeongbuk-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
