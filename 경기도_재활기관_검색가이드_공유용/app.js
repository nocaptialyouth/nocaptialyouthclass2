(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("gyeonggi-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // districts/counties of Gyeonggi-do
  const districtOrder = [
    "수원시", "고양시", "성남시", "용인시", "부천시",
    "안산시", "남양주시", "안양시", "화성시", "평택시",
    "의정부시", "파주시", "김포시", "광명시", "군포시",
    "광주시", "이천시", "양주시", "안성시", "구리시",
    "의왕시", "포천시", "하남시", "동두천시", "과천시",
    "여주시", "양평군", "가평군", "연천군"
  ];
  ...
  const homepageSearchUrl = (item) =>
    `https://search.naver.com/search.naver?query=${encodeURIComponent(item.name + " 홈페이지")}`;
...
* 관련 유튜브: ${youtubeUrl(item)}`;
    ...
    elements.homepageDialogContent.innerHTML = `
      ...
      <div class="homepage-dialog-body">
        <p class="homepage-dialog-desc">
          공식 웹사이트 바로가기 또는 연결 불안정에 대비한 네이버 검색 이동을 선택하실 수 있습니다.
        </p>
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("gyeonggi-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `경기도 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
