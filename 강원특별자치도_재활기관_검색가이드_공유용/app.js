(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("gangwon-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  // 18 districts/counties of Gangwon-do
  const districtOrder = [
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("gangwon-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `강원 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
